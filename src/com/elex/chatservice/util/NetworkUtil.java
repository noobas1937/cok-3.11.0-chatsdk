...
//		LogUtil.printVariablesWithFuctionName(Log.VERBOSE, LogUtil.TAG_WS_STATUS, "avgRtt", avgRtt, "lossPercent", lossPercent);
	 ...
	 * 在本地先试试，看看结果是否稳定
	 */
	public static void testServerAndSaveResult(ArrayList<StandaloneServerInfo> servers, IWebSocketStatusListener statusListener)
	{
		...
		int i = 0;
		for (StandaloneServerInfo server : servers)
		{
		...
		if(servers.size() > 0){
			StandaloneServerInfoManager.sortServers(servers);
			StandaloneServerInfoManager.getInstance().save(network, servers);
		}
	}

	private static String getWebsocketHealthStatus(StandaloneServerInfo server)
	{
