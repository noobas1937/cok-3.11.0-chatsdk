...
import com.elex.chatservice.view.recyclerrefreshview.pulltoswipeview.PullToRefreshSwipeRecyclerView;
...
public abstract class AbstractRecyclerActivity extends RecyclerActionBarActivity implements OnClickListener
{
	protected AbstractRecyclerAdapter			adapter					= null;
	protected SwipeMenuRecyclerListView			mRecyclerView;
	private FrameLayout							channel_list_layout;
	protected PullToRefreshSwipeRecyclerView	channelListPullView;
...
//		LogUtil.printVariablesWithFuctionName(Log.INFO, LogUtil.TAG_DEBUG, "onCreate 3");
		...
		mRecyclerView.setSwipMenuHandle(createSwipeMenuHandle());
...
//		LogUtil.printVariablesWithFuctionName(Log.INFO, LogUtil.TAG_DEBUG, "onCreate END");
	...
	public SwipeMenuRecyclerListView getListView()
	{
	...
	private OnRefreshListener<SwipeMenuRecyclerListView>	refreshlistener			= new OnRefreshListener<SwipeMenuRecyclerListView>()
																					{
																						@Override
																						public void onPullDownToRefresh(PullToRefreshBase<SwipeMenuRecyclerListView> refreshView)
																						{
																						}

																						@Override
																						public void onPullUpToRefresh(PullToRefreshBase<SwipeMenuRecyclerListView> refreshView)
																						{
																							if (adapter != null)
																							{
	...
	private void initView()
	{
		...
		channelListPullView = (PullToRefreshSwipeRecyclerView) findViewById(R.id.channelListPullView);
		mRecyclerView = channelListPullView.getRefreshableView();
...
//		LogUtil.printVariablesWithFuctionName(Log.INFO, LogUtil.TAG_DEBUG, "initView 6");
		mRecyclerView.setMenuCreator(initSwipeMenuCreator());
		mRecyclerView.setOnMenuItemClickListener(initSwipeMenuItemClickListener());
...
//		LogUtil.printVariablesWithFuctionName(Log.INFO, LogUtil.TAG_DEBUG, "initView 7");
	...
	{
		dataChanged = true;
		if (ChatServiceController.getRecyclerListActivity() != null)
		{
			try
			{
				ChatServiceController.getRecyclerListActivity().reload();
				dataChanged = false;
			}
			catch (Exception e)
			{
	...
	public static void onMsgAdded(ChatChannel channel)
	{
		...
		if (ChatServiceController.getRecyclerListActivity() != null)
		{
			AbstractRecyclerAdapter adapter = ChatServiceController.getRecyclerListActivity().adapter;
			if (adapter != null && adapter.list != null)
			{
	...
	{
		dataChanged = true;
		if (ChatServiceController.getRecyclerMainListActivity() != null && ChatServiceController.getRecyclerMainListActivity().adapter != null)
		{
			ChatServiceController.getRecyclerMainListActivity().refreshChannel();
		}
	}
	...
	public void onResume()
	{
		...
		{
			LogUtil.printVariablesWithFuctionName(Log.DEBUG, LogUtil.TAG_DEBUG, "channelId+AbstractRecyclerActivity",channelId);
			...
			{
				LogUtil.printVariablesWithFuctionName(Log.DEBUG, LogUtil.TAG_DEBUG, "channelId+AbstractRecyclerActivity+赋值=0",channelId);
	...
	{
		if (AbstractRecyclerActivity.this != null)
		{
			AbstractRecyclerActivity.this.runOnUiThread(new Runnable()
			{
	...
	protected void openChannel(ChatChannel channel)
	{
		...
		if (channel.channelType < DBDefinition.CHANNEL_TYPE_OFFICIAL || channel.channelType == DBDefinition.CHANNEL_TYPE_USER)
		{
			if (channel.channelType == DBDefinition.CHANNEL_TYPE_USER || channel.channelType == DBDefinition.CHANNEL_TYPE_CHATROOM)
			{
				...
				{
					ServiceInterface.showRecyclerMaillListActivity(this, false, DBDefinition.CHANNEL_TYPE_USER, channel.channelID,
							false);
	...
	protected Point getCurrentPos()
	{
		...
		int x = mRecyclerView.getFirstVisiblePosition();
		View v = mRecyclerView.getChildAt(0);
		int y = (v == null) ? 0 : (v.getTop() - mRecyclerView.getPaddingTop());
	...
	protected void deleteSysMail(int position)
	{
		...
		{
			if (ChatServiceController.getRecyclerListActivity() != null)
				ChatServiceController.getRecyclerListActivity().actualDeleteSingleSysMail(mailData);
		}
	}
	...
	public void onDestroy()
	{
		...
		if (mRecyclerView != null)
		{
			mRecyclerView.setMenuCreator(null);
			mRecyclerView.setOnMenuItemClickListener(null);
			mRecyclerView = null;
		}
	...
	public abstract SwipeMenuHandle createSwipeMenuHandle();
}
