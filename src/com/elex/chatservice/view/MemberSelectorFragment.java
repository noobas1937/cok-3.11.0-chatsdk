	...
	public void onSelectionChanged()
	{
		...
		if (!ChatServiceController.isCreateChatRoom)
		{
			if(ChatServiceController.getCurrentChannelType() == DBDefinition.CHANNEL_TYPE_CHATROOM)
				...
				titleName = ChatServiceController.getInstance().getUserMailTitle();
		}
		...
		if (size < 0)
			size = 0;
		if(LanguageManager.getLangByKey(LanguageKeys.TITLE_DEFAULT).equals(LanguageManager.getLangByKey(LanguageKeys.TITLE_INVITE))){
			getTitleLabel().setText(titleName + "(" + size + ")");
		}else {
			getTitleLabel().setText(LanguageManager.getLangByKey(LanguageKeys.TITLE_INVITE) + "(" + size + ")");
		}
	...
	public void onViewCreated(View view, Bundle savedInstanceState)
	{
		...
		cs__okBtn.setOnClickListener(new View.OnClickListener()
		{
			public void onClick(View view)
			{
				onConfirm();
				if(ChatServiceController.isCreateChatRoom)
					LogUtil.trackAction("confim_mail_receiver");
			}
		});
