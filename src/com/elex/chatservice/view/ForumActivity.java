...
public class ForumActivity extends MyActionBarActivity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		if (extras != null)
		{
			...
			{
				targetUrl = extras.getString("forumUrl");
			}

			if (extras.getInt("webViewType") != -1)
			{
				webViewType = extras.getInt("webViewType", WEBVIEW_TYPE_FORFUM);
			}
		}
		...
		inflater.inflate(R.layout.cs__forum_fragment, fragmentLayout, true);
		showRightBtn(null);
		generateWebviewUrl();
		bottomButtonBar = (RelativeLayout) findViewById(R.id.bottomButtonBar);
		navButtonLinearLayout = (LinearLayout) findViewById(R.id.cs__NavButtonLinearLayout);
		if (webViewType == WEBVIEW_TYPE_FORFUM)
			titleLabel.setText(LanguageManager.getLangByKey(LanguageKeys.TITLE_FORUM));
		else if (webViewType == WEBVIEW_TYPE_TRANSLATION_OPTIMIZATION)
			titleLabel.setText(LanguageManager.getLangByKey(LanguageKeys.TITLE_TRANSLATION_OPTIMIZATION));
		else if (webViewType == WEBVIEW_TYPE_SHARE)
		{
			titleLabel.setText(LanguageManager.getLangByKey(LanguageKeys.MAIL_TABNAME_FIGHT));
			navButtonLinearLayout.setVisibility(View.GONE);
		}

		webViewContainer = (RelativeLayout) findViewById(R.id.messagesListLayout);
		webView = (WebView) findViewById(R.id.cs__webView);
		...
		backButton = (ImageButton) findViewById(R.id.cs__backButton);
		forwardButton = (ImageButton) findViewById(R.id.cs__forwardButton);
		refreshButton = (ImageButton) findViewById(R.id.cs__refreshButton);
		translateButton = (Button) findViewById(R.id.cs__translateButton);
		closeButton = (Button) findViewById(R.id.cs__closeButton);
		...
		fragmentLayout.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener()
		{
	...
	private void generateWebviewUrl()
	{
		...
		else if (webViewType == WEBVIEW_TYPE_TRANSLATION_OPTIMIZATION)
		{
			if (ChatServiceController.getInstance().isInDummyHost())
			{
//				loginUrl = "https://calendar.google.com/calendar/embed?src=zh.hong_kong%23holiday%40group.v.calendar.google.com&ctz=Asia/Hong_Kong";
	...
	private void loadFirstPage()
	{
		...
		if (!hasLogined)
		{
			...
			else if (webViewType == WEBVIEW_TYPE_TRANSLATION_OPTIMIZATION || webViewType == WEBVIEW_TYPE_SHARE)
			{
				loadUrl(loginUrl);
			}
