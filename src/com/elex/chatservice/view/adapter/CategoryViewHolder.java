...
public class CategoryViewHolder
{
	...
	public CategoryViewHolder(View view)
	{
		...
		if(ConfigManager.getInstance().needRTL())
			ViewHelper.setRotation(list_item_arrow, 180.0f);
	...
	protected void showUnreadCountText(ChannelListItem item ,Context context)
	{
		...
		if(item instanceof ChatChannel && (((ChatChannel)item).channelID.equals(MailManager.CHANNELID_RESOURCE) || ((ChatChannel)item).channelID.equals(MailManager.CHANNELID_MONSTER)) && unreadCount>0)
		{
			unread_text.setText("");
			if(unread_dot!=null)
	...
	protected void adjustSize(Context context)
	{
		...
		if (ConfigManager.getInstance().scaleFontandUI && ConfigManager.scaleRatio > 0 && !adjustSizeCompleted)
		{
			...
			int picWidthDP = 50;
			int width = (int) (ScaleUtil.dip2px(context, picWidthDP)
					* ConfigManager.scaleRatio * getScreenCorrectionFactor(context));
			LogUtil.printVariablesWithFuctionName(Log.INFO, LogUtil.TAG_DEBUG, "width",width);

			if (this instanceof MailViewHolder)
			{
				item_pic_layout.setLayoutParams(new RelativeLayout.LayoutParams(width, (int) (ScaleUtil.dip2px(context, picWidthDP)
						* ConfigManager.scaleRatio * getScreenCorrectionFactor(context))));
			}
			else
			{
	...
	{
		 ScaleUtil.adjustTextSize(item_title, ConfigManager.scaleRatio * getFontScreenCorrectionFactor(ChatServiceController.getCurrentActivity()));
		 ScaleUtil.adjustTextSize(unread_text, ConfigManager.scaleRatio * getFontScreenCorrectionFactor(ChatServiceController.getCurrentActivity()));
	}
