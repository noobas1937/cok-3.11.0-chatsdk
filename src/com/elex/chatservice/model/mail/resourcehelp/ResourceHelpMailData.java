		...
		if (!getContents().equals(""))
		{
			try
			{
				...
				if(needParseByForce)
					return;
				String tempContent = "";
				List<ResourceHelpReward> list = new ArrayList<ResourceHelpReward>();
				if (detail.getReward() != null && detail.getReward().size() > 0)
				{
					...
					if (helpReward != null)
					{
						...
						if (value > 0)
						{
							String numText = MathUtil.getFormatNumber(value);
							tempContent = "[" + icon + "]" + " + " + numText;
							
							ResourceHelpReward reward = new ResourceHelpReward();
							reward.setIcon(icon);
							reward.setNum(numText);
							list.add(reward);
						}
					}
				...
				if(MailManager.cocosMailListEnable)
				{
					...
					{
						try {
							contentText = JSON.toJSONString(list);
