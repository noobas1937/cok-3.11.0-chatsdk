...
public class StandaloneServerInfo implements Serializable
{
	...
	public StandaloneServerInfo(String protocol, String address, String port)
	{
	...
	public boolean equalTo(StandaloneServerInfo info)
	{
		return info.address.equals(address) && info.protocol.equals(protocol) && info.port.equals(port);
	}

	public void updateTestResult(StandaloneServerInfo newInfo)
	{
		lastTestTime = TimeManager.getInstance().getCurrentTimeMS();
		if(!isTestTooOld())
		{
	...
	{
		long timeoutMS = (!SwitchUtils.mqttEnable ?
				WebSocketManager.getInstance().networkOptimizationTimeout :
					MqttManager.getInstance().networkOptimizationTimeout) * 1000;
