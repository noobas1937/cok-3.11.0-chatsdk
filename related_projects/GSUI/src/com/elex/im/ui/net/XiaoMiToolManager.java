...
import com.elex.im.ui.util.FileVideoUtils;
		...
		public void onRecordFinished(String localPath, long length) {
			...
			if(sendCurrentRecord)
			{
				if(mLength>=1800)
				{
					...
					GSController.sendDummyAudioMsg(length,sendLocalTime);
			...
			if(fromGuid.equals(gUid))
			{
				if (IMCore.hostActivity != null) {
					IMCore.hostActivity.runOnUiThread(new Runnable()
    				{
    					...
    					public void run()
    					{
    						CokConfig.getInstance().getAllianceChannel().sendAudioMsgToServer(url, sendLocalTime);
//							CokConfig.getInstance().getAllianceChannel().sendMsg(mLength + "", false, false, url);
    					}
    				});
                }
			}
