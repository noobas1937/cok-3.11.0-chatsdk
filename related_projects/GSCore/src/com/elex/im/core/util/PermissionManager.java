...
public class PermissionManager
{
	...
	{
		if (ActivityCompat.shouldShowRequestPermissionRationale(IMCore.hostActivity,
				Manifest.permission.WRITE_EXTERNAL_STORAGE))
		{
...
//					LanguageKeys.PERMISSION_REQUEST_WRITE_SD_CARD);
	...
	{
		ActivityCompat.requestPermissions(IMCore.hostActivity, PERMISSIONS_STORAGE, MY_PERMISSIONS_REQUEST_EXTERNAL_STORAGE);
	}

	public static boolean checkXMRecordPermission(Activity activity)
	{
		if (!isXMRecordPermissionsAvaiable(activity))
		{
			if (shouldShowXMRecordRequestPermissionRationale(activity))
			{
				String notify = LanguageManager.getLangByKey(LanguageKeys.PERMISSION_REQUEST_RECORD_VOICE);
//				MenuController.showAllowPermissionConfirm(activity, notify,
//						LanguageKeys.PERMISSION_REQUEST_RECORD_VOICE);
			}
			else
			{
				actualGetXMRecordPermissions(activity);
			}
	...
	private static void actualGetXMRecordPermissions(Activity activity)
	{
		ActivityCompat.requestPermissions(activity, PERMISSIONS_XM_RECORD,
...
//					DBManager.getInstance().onRequestPermissionsResult();
	...
	public static void onNotifyPermissionConfirm(String permissionKey, Activity activity)
	{
...
//		}
	...
	public static boolean checkAllianceSharePermissions(Activity activity)
	{
		if (!isAllianceSharePermissionsAvaiable(activity))
		{
//			if (shouldShowAllianceShareRequestPermissionRationale(activity))
//			{
//				MenuController.showAllowPermissionConfirm(activity, notify,
//						LanguageKeys.PERMISSION_REQUEST_RECORD_VOICE);
//			}
//			else
//			{
			actualGetAllianceSharePermissions(activity);
//			}
