...
public class WebSocketImpl implements WebSocket {
	...
	public void decode( ByteBuffer socketBuffer ) {
		...
		if (DEBUG)
		{
			LogUtil.printVariablesWithFuctionName(Log.VERBOSE, LogUtil.TAG_WS_STATUS, "process("
					+ socketBuffer.remaining()
					+ "): {"
					+ (socketBuffer.remaining() > 1000 ? "too big to display" : new String(socketBuffer.array(), socketBuffer.position(),
							socketBuffer.remaining())) + "}");
		}
	...
	private void decodeFrames( ByteBuffer socketBuffer ) {
		...
		try {
			...
			for( Framedata f : frames ) {
				if (DEBUG)
				{
					LogUtil.printVariablesWithFuctionName(Log.VERBOSE, LogUtil.TAG_WS_STATUS, "matched frame: " + f);
				}
...
//		LogUtil.printVariablesWithFuctionName(Log.INFO, LogUtil.TAG_WS_STATUS, "readystate", readystate.toString(), "key", key, "key", key);
	...
	public void eot() {
//		LogUtil.printVariablesWithFuctionName(Log.INFO, LogUtil.TAG_WS_STATUS, "getReadyState()", getReadyState(), "flushandclosestate",
//				flushandclosestate, "closecode", closecode, "closemessage", closemessage, "closedremotely", closedremotely,
//				"draft.getCloseHandshakeType()", draft.getCloseHandshakeType(), "role", role);
	...
	public void sendFrame( Framedata framedata ) {
		if (DEBUG)
		{
			LogUtil.printVariablesWithFuctionName(Log.VERBOSE, LogUtil.TAG_WS_STATUS, "send frame: " + framedata);
		}
	...
	private void write( ByteBuffer buf ) {
		if (DEBUG)
		{
			LogUtil.printVariablesWithFuctionName(Log.VERBOSE, LogUtil.TAG_WS_STATUS,
					"write(" + buf.remaining() + "): {" + (buf.remaining() > 1000 ? "too big to display" : new String(buf.array())) + "}");
		}
	...
	private void open( Handshakedata d ) {
		if (DEBUG)
		{
			LogUtil.printVariablesWithFuctionName(Log.VERBOSE, LogUtil.TAG_WS_STATUS, "open using draft: "
